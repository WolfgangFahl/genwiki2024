...
"""
    ...
    def generate_restore_script(
        self, output_path: str = "/tmp/restore_wikis.sh"
    ) -> str:
        """
        Generates a bash script to restore original profiwiki LocalSettings.php
        defaults that were damaged by previous patch attempts.
        Undoes wgScriptPath/wgResourceBasePath/wgArticlePath changes,
        only sets wgServer to https.
        see https://github.com/WolfgangFahl/genwiki2024/issues/39
        """
        script_content = f"""#!/bin/bash
# Restore script for Wiki Playgrounds
# Undoes damage from previous Caddy-related patches
# generated by playground.py
# see https://github.com/WolfgangFahl/genwiki2024/issues/39
...
"""
        for god in self.godsList():
            name = god["name"]
            script_content += f"""docker exec "{name}-mw" sed -i 's#^\\$wgServer = .*#\\$wgServer = "https://{self.host}";#' /var/www/html/LocalSettings.php
echo "Restored {name}"
"""
...
def main():
    ...
    parser.add_argument(
        "--restore",
        action="store_true",
        help="Generate restore script (undoes previous patches, sets wgServer to https)",
    )
    parser.add_argument(
    ...
    if args.caddy:
        ...
        handled = True
    if args.restore:
        restore = pg.generate_restore_script()
        pg.color_msg(GREEN, f"✅ {restore} generated.")
        handled = True
    if args.setup:
