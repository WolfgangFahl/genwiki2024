...
<VirtualHost *:80>
    ...
    def generate_caddy_conf(self, output_path: str = "/tmp/Caddyfile") -> str:
        """
        Generates the Caddy configuration file for all wikis
        """
        conf_content = f"""# Proxy configuration for Wiki Playgrounds
# generated by playground.py
# https://github.com/WolfgangFahl/genwiki2024/blob/main/genwiki/playground.py
...
"""
    ...
    def generate_patch_script(self, output_path: str = "/tmp/patch_wikis.sh") -> str:
        """
        Generates a bash script to patch wgServer/wgScriptPath/wgArticlePath
        in all wiki containers to use the correct hostname and https://.
        fixes https://github.com/WolfgangFahl/genwiki2024/issues/36
        """
        script_content = f"""#!/bin/bash
# Patch script for Wiki Playgrounds - fixes wgServer to use https://
# generated by playground.py
# https://github.com/WolfgangFahl/genwiki2024/blob/main/genwiki/playground.py
...
patch_setting() {{
    ...
    local setting=$2
    local value=$3
    docker exec "${{god}}-mw" bash -c "sed -i 's#^\\\\$$setting = .*#\\\\$$setting = \\"$value\\";#' /var/www/html/LocalSettings.php"
}}
...
"""
        for god in self.godsList():
            name = god["name"]
            script_content += f'patch_wiki "{name}"\n'
...
def main():
    ...
    parser.add_argument(
        "--patch",
        action="store_true",
        help="Generate the patch script (fixes wgServer https)",
    )
    parser.add_argument(
